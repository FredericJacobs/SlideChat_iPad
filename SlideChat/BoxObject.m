...
- (NSMutableDictionary *)getValuesInDictionaryForm {
	...
	if(self.objectCreatedTime) {
		[returnDict setObject:[NSString stringWithFormat:@"%f", (double)[self.objectCreatedTime timeIntervalSince1970]] forKey:@"created"];
	}
	if(self.objectUpdatedTime) {
		[returnDict setObject:[NSString stringWithFormat:@"%f", (double)[self.objectUpdatedTime timeIntervalSince1970]] forKey:@"updated"];
	}
